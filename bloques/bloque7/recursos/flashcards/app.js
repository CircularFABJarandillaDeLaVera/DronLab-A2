(() => {
  const data = window.DRONLAB_A2_BLOQUE7_FLASHCARDS;
  const main = document.getElementById('main');
  let current = [...data.flashcards];
  let filter = 'todas';
        ...
        <div class="inner">
          <div class="face front">
            <div class="meta">
              ...
              <span class="tag">${card.dificultad}</span>
              <span class="tag">${card.tipo}</span>
            </div>
  ...
  function render() {
    ...
    main.innerHTML = `
      <section class="panel hero">
        ...
        <p>${data.bloque}</p>
        <div class="stats">
          <span class="pill">${data.total} tarjetas</span>
          <span class="pill">${stats.facil || 0} faciles</span>
          <span class="pill">${stats.media || 0} medias</span>
          <span class="pill">${stats.dificil || 0} dificiles</span>
        </div>
      </section>
      <section class="flashgrid">
        ${visible.length ? visible.map(cardHtml).join('') : '<p class="panel empty">No hay tarjetas para este filtro.</p>'}
      </section>
    `;

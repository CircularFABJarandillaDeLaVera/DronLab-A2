(() => {
  const data = window.DRONLAB_BLOQUE1;
  ...
  const notesKey = 'dronlab_a2_bloque1_notes';
  ...
  function renderSlide(slide, i) {
    ...
    const body = i === 0
      ? `<div class="hero-grid"><div class="hero-copy"><p class="eyebrow">CURSO UAS - A2</p><h1>${escapeHtml(slide.titulo)}</h1><p class="subtitle">${escapeHtml(slide.objetivo)}</p><div class="institutional-pill"><img src="../../assets/branding/logo-circular-fab.png" alt=""><span>Circular FAB Jarandilla de la Vera</span></div><div class="hero-tags"><span>Ver</span><span>Entender</span><span>Aplicar</span><span>Recordar</span></div></div><div class="drone-visual" aria-label="Diagrama de tramites"><div class="system-card"><span class="icon">A2</span><b>Bloque Base</b><small>Requisitos y tramites</small></div></div></div>`
      : `<p class="eyebrow">${escapeHtml(slide.tipo)}</p><h2>${escapeHtml(slide.titulo)}</h2><div class="compare compact"><article><div class="badge">OBJETIVO</div><h3>${escapeHtml(slide.objetivo)}</h3><p>${escapeHtml(slide.contenido)}</p></article><article><div class="badge alt">RESUMEN</div><h3>Idea clave</h3><p>${escapeHtml(slide.resumen)}</p></article></div><div class="callout"><b>Profesor Plus:</b> ${escapeHtml(pp.que_decir || slide.guion)}</div>`;
    return `
      <section class="slide ${i === 0 ? 'active slide-hero' : ''}" data-title="${escapeHtml(slide.titulo)}" data-section="${escapeHtml(slide.tipo)}">
        ...
        <aside class="notes">
          ...
          <b>Material:</b> ${escapeHtml(pp.material || slide.material)}<br>

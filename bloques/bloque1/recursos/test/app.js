  ...
  function render() {
    ...
    main.innerHTML = `
      <section class="panel hero">
        ...
        <p>${data.bloque}</p>
        <div class="stats">
          <span class="pill">${data.total} preguntas</span>
          <span class="pill">${count.facil || 0} faciles</span>
          <span class="pill">${count.media || 0} medias</span>
          <span class="pill">${count.dificil || 0} dificiles</span>
        </div>
      </section>
      ...
      ${visible.map((question, index) => `
        <section class="panel question" data-id="${question.id}">
          <div class="meta">
            ...
            <span class="tag">${question.dificultad}</span>
            <span class="tag">${question.tema}</span>
          </div>
          <h2>${index + 1}. ${question.pregunta}</h2>
          <div class="opts">
            ${question.respuestas.map((answer, answerIndex) => `<button class="opt" data-id="${question.id}" data-answer="${answerIndex}">${answer}</button>`).join('')}
          </div>
          <div class="feedback">
            <p><b>Explicacion:</b> ${question.explicacion}</p>
            <p><b>Error habitual:</b> ${question.error_habitual}</p>
            <p><b>Referencia:</b> ${question.referencia_concepto}</p>
          </div>
        </section>
      `).join('')}
    `;
